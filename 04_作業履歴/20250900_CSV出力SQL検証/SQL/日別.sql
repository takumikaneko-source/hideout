-- 日別 滋賀交通(02000046) 琵琶湖タクシー本社(00000006)

SET @cstm_cd = '02000046';
SET @eigyosyo_cd = '00000006';
SET @month = '2024/03';
...
eft AS (
    ...
    FROM
        taxicloud.eigyo_flat AS ef
    WHERE
        ef.cstm_cd = @cstm_cd
        AND ef.eigyosyo_cd = @eigyosyo_cd
        AND ef.sales_month = @month
        AND ef.geisya_flg = '1'
    GROUP BY
        ef.cstm_cd
        ,ef.eigyosyo_cd
...
SELECT
	...
	uf.sales_date AS '日付',
    -- No 2 画面表示項目
	CASE WHEN uf.eigyosyo_target_date = 'E' THEN 0 ELSE IFNULL(MAX(CONVERT(uf.eigyosyo_target_date,DECIMAL)), 0) END AS '目標[円]',
    -- No 3 画面表示項目
    ...
    -- No 7 画面表示項目
    CASE WHEN uf.eigyosyo_target_date = 'E' THEN 0 ELSE IFNULL(ROUND(SUM(uf.soeisyu) / MAX(CONVERT(uf.eigyosyo_target_date,DECIMAL)) * 100, 2), 0) END AS '目標達成率[％]',
    -- No 8 画面表示項目
    IFNULL(ROUND(SUM(uf.soeisyu) / SUM(ABS(TIMESTAMPDIFF(MINUTE, uf.nyuko_dtm, uf.syukko_dtm)) / 60)), 0) AS '売上/時間[円]',
    -- No 9 画面表示項目
    IFNULL(SUM(uf.sa_eigyo_cnt), 0) AS '営業回数[回]',
    -- No 10 画面表示項目
    IFNULL(eft.geisya_cnt, 0) AS '迎車回数[回]',
    -- No 11 画面表示項目
    IFNULL(COUNT(DISTINCT uf.jomuin_cd), 0) AS '乗務人数[人]',
    -- No 12 画面表示項目
    IFNULL(ROUND(SUM(CONVERT(uf.sa_eigyosoko_km,DECIMAL(4,1))) / SUM(CONVERT(uf.sa_zensoko_km,DECIMAL(4,1))) * 100, 2), 0) AS '実車率[％]',
    -- No 13
    IFNULL(ROUND(SUM(ABS(TIMESTAMPDIFF(MINUTE, uf.nyuko_dtm, uf.syukko_dtm))), 0), 0) AS '乗務時間[分]',
    -- No 14
    ...
    -- No 35
	IFNULL(SUM(uf.total_okikae_kin_flg), 0) AS '置換料金合計[円]',
    -- No 36
