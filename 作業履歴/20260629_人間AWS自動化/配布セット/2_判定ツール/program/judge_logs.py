...
try:
    ...
    print("[エラー] 必要なパッケージが未導入です: " + str(_e))
    print("  このフォルダで次を実行してください:  pip install -r requirements.txt")
    print("  （配布セット直下の「セットアップ.bat」でもまとめて導入できます）")
    raise SystemExit(1)

# ツールのルート（settings.py がある場所）を基準にする。
# 本体は program/ 配下にあるが、settings.py・ナレッジ・OUTPUT はその1つ上（ツール直下）にある。
...
if os.path.exists(os.path.join(_HERE, "settings.py")):
    ...
    BASE_DIR = _HERE
sys.path.insert(0, BASE_DIR)   # settings.py を確実に読めるように
...
HELP = """アラートログ判定ツール (judge_logs.py)

【役割】
  ログ収集ツールが転記したエラーログ(D列)を読み、対応要否(要/静観/不要)と
  影響度(大/中/小)をAIが判定して、判定済みExcelのG〜K列に書き込みます。
  最終判断は運用チームの目視(E列)で行います。AIは参考情報です。

【使い方】
  1) 事前に  pip install -r requirements.txt （配布セットの「セットアップ.bat」でも可）
  2) サブスクで使う場合: `claude` を導入し一度ログイン（settings.py の BACKEND="claude_code"）
  3) settings.py を必要に応じて編集（入力ファイル・MAX_ROWS など）
  4) 「判定実行.bat」をダブルクリック、または  python program\judge_logs.py

【主な設定 (settings.py)】
  INPUT_XLSX   … 入力(=収集ツールの出力ファイル)
  BACKEND      … claude_code(サブスク) / api(従量課金・要APIキー)
  JUDGE_MODE   … orchestration(案B) / pipeline(案A)
  MAX_ROWS     … 1回の処理行数（まずは小さい値で試す）
  DRY_RUN=True … AIを呼ばず機械判定だけ試す（費用ゼロ・claude不要）

【出力】
  ・…_判定済.xlsx（G〜K列: 対応要否/影響度/理由/確信度/判定方式）
  ・AI判定_運用差分記録.xlsx（AI判定と運用判断の差分を蓄積する永続ファイル）

【困ったとき】
  ・環境が整っているかは、配布セット直下の「環境チェック.bat」で点検できます。
  ・claude が見つからない: npm install -g @anthropic-ai/claude-code → `claude` でログイン。
"""
