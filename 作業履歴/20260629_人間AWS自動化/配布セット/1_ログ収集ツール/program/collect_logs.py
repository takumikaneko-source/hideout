...
# ツールのルート（settings.py がある場所）を基準にする。
#   本体は program/ 配下にあるが、settings.py・INPUT・OUTPUT はその1つ上（ツール直下）にある。
#   program/配下でも、従来どおり直下に置いた場合でも、どちらでも動くよう自動判定する。
_HERE = os.path.dirname(os.path.abspath(__file__))
if os.path.exists(os.path.join(_HERE, "settings.py")):
    _BASE_DIR = _HERE
elif os.path.exists(os.path.join(os.path.dirname(_HERE), "settings.py")):
    _BASE_DIR = os.path.dirname(_HERE)
else:
    _BASE_DIR = _HERE
sys.path.insert(0, _BASE_DIR)   # settings.py を確実に読めるように
try:
...
# 相対パス（".\INPUT\..." 等）は、ツールのルート（settings.py のある場所）基準で解決する。
# こうしておくと、別フォルダから実行しても・配布フォルダごと移動しても壊れない。
# （OUTPUT_XLSX はファイル名としても使うため、ここでは解決しない）
for _k in ("INPUT_XLSX", "OUTPUT_DIR", "CA_BUNDLE"):
    _v = globals().get(_k)
    if _v and not os.path.isabs(_v):
        globals()[_k] = os.path.normpath(os.path.join(_BASE_DIR, _v))
...
def main():
    ...
    try:
        ...
        log(f"[NG  ] 必要なパッケージが未導入です: {e}")
        log("       このフォルダで次を実行してください:  pip install -r requirements.txt")
        log("       （配布セット直下の「セットアップ.bat」でもまとめて導入できます）")
        sys.exit(1)
...
HELP = """CloudWatch ログ収集ツール (collect_logs.py)

【役割】
  Slack通知アラート集計xlsx を読み、まだログを取っていない行(D列が空)について、
  アラーム名からAWSプロファイルを判定し、該当時刻のCloudWatchログを取得して
  出力xlsxの D列(ログ本文)と F列(コンソールへのリンク)に転記します。
  AIは使いません。AWSへは読み取りのみ。資格情報はAIに渡しません。

【使い方】
  1) 事前に  pip install -r requirements.txt （配布セットの「セットアップ.bat」でも可）
  2) 対象の集計xlsx を INPUT フォルダに置く
  3) settings.py を編集（INPUT_XLSX、PREFIX_TO_PROFILE を自分の環境に合わせる）
  4) AWSにスイッチロール/MFAでログインできる状態にする
  5) 「収集実行.bat」をダブルクリック、または  python program\collect_logs.py

【主な設定 (settings.py)】
  INPUT_XLSX        … 入力xlsx のパス
  PREFIX_TO_PROFILE … アラーム名の接頭辞→AWSプロファイル
  SHEET_NAME        … 対象シート（空=当月を自動）
  WINDOW_MIN        … 時刻の前後 何分を検索するか（既定5）
  MAX_ROWS          … 1回の処理行数（0=制限なし）

【困ったとき】
  ・環境が整っているかは、配布セット直下の「環境チェック.bat」で点検できます。
  ・処理される行が少ない: 入力xlsxで「A・Bがあり D列が空」の行だけが対象です。
"""
